...
export default function Home() {
  ...
  const firstName=(profile?.fullName||profile?.email||"Emmanuel").trim().split(/\s+/)[0];
  ...
  if (profile?.role === "Admin") navItems.push(["users", "users", "Users"], ["audit", "audit", "Audit Logs"], ["backup", "backup", "Backup Center"], ["system", "system", "System Information"]);
  ...
  return <main className="app-shell">
    ...
    <aside id="main-sidebar" className={`sidebar mobile-drawer ${mobileNavOpen?"open":""}`}><div className="brand"><ChurchBrand inverse/></div><nav aria-label="Main navigation"><p className="nav-section-label">Workspace</p>{navItems.map(([key, icon, label]) => <button key={key} title={label} className={`nav-item ${view === key ? "active" : ""}`} aria-current={view===key?"page":undefined} onClick={() => { setView(key); setMobileNavOpen(false); }}><span className="nav-icon"><AppIcon name={icon}/></span><span className="nav-label">{label}</span></button>)}<button className="nav-item logout-nav" onClick={()=>{setMobileNavOpen(false);void signOut();}}><span className="nav-icon"><AppIcon name="logout"/></span><span className="nav-label">Logout</span></button></nav><div className="sidebar-foot">{profile&&<UserProfileIndicator name={profile.fullName} email={profile.email} role={profile.role}/>}<div className="connection-state"><div className={`sync-dot ${connected ? "" : "pending"}`} /><span>{connected ? "Database connected" : "Connection pending"}</span></div></div></aside>
    <section className="workspace">
      <header className="topbar"><div className="welcome-heading"><p className="eyebrow">{new Intl.DateTimeFormat("en", { weekday: "long", month: "long", day: "numeric" }).format(new Date())}</p><h1>{headings[view][0]}</h1><p className="subhead">{headings[view][1]}</p></div><div className="topbar-actions">{profile&&<UserProfileIndicator name={profile.fullName} email={profile.email} role={profile.role} compact/>}{headerActions()}</div></header>
      ...
      {view === "dashboard" && <DashboardView data={data} isAdmin={profile?.role === "Admin"} dataLoading={loading} onViewTransactions={() => setView("transactions")} />}
      ...
      {view === "backup" && profile?.role === "Admin" && <BackupCenterView profile={profile}/>}
      {view === "system" && profile?.role === "Admin" && <SystemInformationView profile={profile}/>}
