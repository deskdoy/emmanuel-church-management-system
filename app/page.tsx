...
type View = "dashboard" | "transactions" | "payables" | "accounts" | "projects" | "reports" | "users" | "audit" | "settings";
...
function TransactionTable({ transactions, emptyMessage, canEdit, onDetails, onEdit }: { transactions: Transaction[]; emptyMessage: string; canEdit: boolean; onDetails: (transaction:Transaction)=>void; onEdit: (transaction:Transaction)=>void }) {
  return <div className="table-wrap"><table><thead><tr><th>Date</th><th>Description</th><th>Type</th><th>Account</th><th>Category</th><th className="num">Amount</th><th>Action</th></tr></thead><tbody>
    {[...transactions].sort((a, b) => b.date.localeCompare(a.date)).map(transaction => <tr key={transaction.id}>
      <td>{dateLabel(transaction.date)}</td><td><b>{transaction.description || "—"}</b><small>{transaction.reference || transaction.paymentMethod}</small></td><td><span className={`status ${transaction.type.toLowerCase()}`}>{transaction.type}</span></td><td>{transaction.account}</td><td>{transaction.category}</td><td className={`num ${transaction.type === "Income" ? "income-text" : ""}`}>{transaction.type === "Income" ? "+" : "-"}{peso(transaction.moneyIn || transaction.moneyOut)}</td><td><div className="row-actions"><button className="table-action" onClick={()=>onDetails(transaction)}>Details</button>{canEdit&&<button className="table-action" onClick={()=>onEdit(transaction)}>Edit</button>}</div></td>
    </tr>)}
    {!transactions.length && <tr><td colSpan={7} className="blank-row">{emptyMessage}</td></tr>}
  </tbody></table></div>;
}

function TransferHistory({ transfers }: { transfers: AccountTransfer[] }) {
  return <section className="panel transfer-history-panel"><div className="panel-head"><div><p className="eyebrow">{transfers.length} transfer{transfers.length===1?"":"s"}</p><h2>Transfer history</h2></div><span className="status transfer">Balance only</span></div><div className="table-wrap"><table><thead><tr><th>Date</th><th>From account</th><th>To account</th><th>Reference</th><th>Recorded by</th><th className="num">Amount</th></tr></thead><tbody>{transfers.map(transfer=><tr key={transfer.id}><td>{dateLabel(transfer.date)}</td><td><b>{transfer.fromAccount}</b></td><td><b>{transfer.toAccount}</b><small>{transfer.notes||"Internal account transfer"}</small></td><td>{transfer.reference||"—"}</td><td>{transfer.recordedByName}</td><td className="num">{peso(transfer.amount)}</td></tr>)}{!transfers.length&&<tr><td colSpan={6} className="blank-row">No account transfers have been recorded.</td></tr>}</tbody></table></div></section>;
}
...
export default function Home() {
  ...
  const save = async (payload: CashFlowMutation) => {
    ...
    try {
      ...
      if (payload.action === "updateTransaction") await updateTransaction(payload.transaction, data);
      if (payload.action === "addPayable") await addPayable(payload.payable);
      if (payload.action === "recordPayablePayment") await recordPayablePayment(payload.payment);
      if (payload.action === "addTransfer") await addTransfer(payload.transfer, data);
  ...
  const headings: Record<View, [string, string]> = {
    dashboard: ["Good morning, Emmanuel.", "Here’s how your cash is moving today."], transactions: ["Transactions", "Record and review money in, money out, and transfers."], payables: ["Payables", "Manage commitments, balances, and payments due."], accounts: ["Accounts", "Manage cash and bank accounts without losing history."], projects: ["Projects", "Plan and follow church initiatives in one place."], reports: ["Reports", "Generate statements and review cash flow analytics."], users: ["Users", "Manage approved users, roles, and account status."], audit: ["Audit logs", "Review secured, immutable records of activity across the system."], settings: ["Settings", "Review your account and application configuration."],
  };
  const navItems: [View, string, string][] = [["dashboard", "⌂", "Dashboard"], ["transactions", "↕", "Transactions"], ["payables", "◷", "Payables"], ["accounts", "◎", "Accounts"], ["projects", "◇", "Projects"], ["reports", "▤", "Reports"]];
  if (profile?.role === "Admin") navItems.push(["users", "♙", "Users"], ["audit", "◈", "Audit Logs"]);
  navItems.push(["settings", "⚙", "Settings"]);
  const showFinanceNotice = !canWriteFinance && ["transactions", "payables"].includes(view);
  const headerActions = () => {
    if (["dashboard", "transactions"].includes(view)) return <button className="primary-button" disabled={!canWriteFinance} onClick={() => openTransaction("Income")}>＋ New Transaction</button>;
  ...
  return <main className="app-shell">
    ...
    <aside id="main-sidebar" className={`sidebar mobile-drawer ${mobileNavOpen?"open":""}`}><div className="brand"><span className="brand-mark">E</span><span>Emmanuel<br /><b>Cash Flow</b></span></div><nav aria-label="Main navigation">{navItems.map(([key, icon, label]) => <button key={key} className={`nav-item ${view === key ? "active" : ""}`} onClick={() => { setView(key); setMobileNavOpen(false); }}><Icon>{icon}</Icon>{label}</button>)}<button className="nav-item logout-nav" onClick={()=>{setMobileNavOpen(false);void signOut();}}><Icon>↪</Icon>Logout</button></nav><div className="sidebar-foot"><div className="connection-state"><div className={`sync-dot ${connected ? "" : "pending"}`} /><span>{connected ? "Database connected" : "Connection pending"}</span></div><div className="signed-in-user"><b>{profile?.fullName || profile?.email}</b><small>{profile?.role}</small></div></div></aside>
    <section className="workspace">
      ...
      {view === "projects" && <ProjectsView canManage={!!profile&&["Admin","Pastor","Secretary"].includes(profile.role)}/>}
      {view === "reports" && <ReportsView data={data} />}
      {view === "users" && profile?.role === "Admin" && <UsersView currentUserId={profile.id}/>}
      {view === "audit" && profile?.role === "Admin" && <AuditLogsView />}
      {view === "settings" && profile && <SettingsView profile={profile} connected={connected}/>}
    </section>
...
function TransactionForm({ data, initialType, transaction, saving, onSubmit }: { data: Data; initialType: TransactionType; transaction?:Transaction; saving: boolean; onSubmit: (payload: CashFlowMutation) => Promise<void> }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const value:Transaction={id:transaction?.id||crypto.randomUUID(),source,date:String(form.get("date")||""),type,account:String(form.get("account")||""),category,description:transaction?.description||vendor||category,vendor,moneyIn:type==="Income"?amount:0,moneyOut:type==="Expense"?amount:0,paymentMethod:transaction?.paymentMethod||"Cash",reference:String(form.get("reference")||""),notes:String(form.get("notes")||""),specifiedDetails:String(form.get("specifiedDetails")||""),createdAt:transaction?.createdAt||new Date().toISOString()};
    void onSubmit({action:transaction?"updateTransaction":"addTransaction",transaction:value});
  };
  return <form className="record-form" onSubmit={submit}><div className="segmented transaction-types"><button type="button" disabled={!!transaction} className={type==="Income"?"selected":""} onClick={()=>changeType("Income")}>Money In</button><button type="button" disabled={!!transaction} className={type==="Expense"?"selected expense":""} onClick={()=>changeType("Expense")}>Money Out</button><button type="button" disabled={!!transaction} className={type==="Transfer"?"selected transfer":""} onClick={()=>changeType("Transfer")}>Transfer</button></div>{type==="Transfer"?<div className="form-grid"><label>From Account<select name="fromAccountId" required>{accounts.map(account=><option key={account.id} value={account.id}>{account.name} · {peso(account.currentBalance)}</option>)}</select></label><label>To Account<select name="toAccountId" defaultValue={accounts[1]?.id||""} required>{accounts.map(account=><option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label>Amount (PHP)<input name="amount" type="number" min="0.01" step="0.01" placeholder="0.00" required /></label><label>Date<input name="date" type="date" defaultValue={today} required /></label><label>Reference<input name="reference" placeholder="Transfer reference no." /></label><label className="full">Notes<textarea name="notes" rows={3} placeholder="Optional transfer notes" /></label><p className="form-help full">Transfers move money between accounts only. They are excluded from income, expense, net cash flow, analytics, and reports.</p></div>:<div className="form-grid"><label>Date<input name="date" type="date" defaultValue={transaction?.date||today} required /></label><label>Account<select name="account" defaultValue={transaction?.account} required>{accounts.map(account=><option key={account.id} value={account.name}>{account.name}</option>)}</select></label><label>Category<select name="category" value={categoryName} onChange={event=>setCategoryName(event.target.value)} required>{categories.map(category=><option key={category.id} value={category.name}>{category.name}</option>)}</select></label><label>Amount (PHP)<input name="amount" type="number" min="0.01" step="0.01" defaultValue={transaction?transaction.moneyIn||transaction.moneyOut:undefined} placeholder="0.00" required /></label>{isOtherCategory(categoryName)&&<label className="full">Specify details<input name="specifiedDetails" defaultValue={transaction?.specifiedDetails||extractSpecifiedDetails(transaction?.notes||"")} placeholder="Describe the other income or expense" required /></label>}{type==="Expense"&&<label className="full">Vendor / Payee<input name="vendor" defaultValue={transaction?.vendor||""} placeholder="Who was paid?" required /></label>}<label>Reference<input name="reference" defaultValue={transaction?.reference||""} placeholder="Receipt or reference no." /></label><label className="full">Notes<textarea name="notes" rows={3} defaultValue={stripSpecifiedDetails(transaction?.notes||"")} placeholder="Optional notes" /></label></div>}{transaction&&<p className="audit-note">This saves over the original record ID. Supabase audit logs retain the previous and updated values.</p>}<button className="primary-button form-submit" disabled={saving||accounts.length<(type==="Transfer"?2:1)||(type!=="Transfer"&&!categories.length)}>{saving?"Saving…":transaction?"Save transaction changes":type==="Income"?"Save money in":type==="Expense"?"Save money out":"Save transfer"}</button></form>;
}
