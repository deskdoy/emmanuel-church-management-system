...
import { AccessRequestsView } from "../src/components/AccessRequestsView";
...
type View = "dashboard" | "transactions" | "income" | "expenses" | "payables" | "accounts" | "reports" | "analytics" | "access-requests" | "audit";
...
export default function Home() {
  ...
  if (profile?.role === "Admin") navItems.push(["access-requests", "♙", "Access Requests"], ["audit", "◇", "Audit Logs"]);
  const showFinanceNotice = !canWriteFinance && ["transactions", "income", "expenses", "payables"].includes(view);
  const headerActions = () => {
    if (["dashboard", "transactions"].includes(view)) return <div className="header-actions"><button className="primary-button" disabled={!canWriteFinance} onClick={() => openTransaction("Income")}>＋ Record Income</button><button className="secondary-button" disabled={!canWriteFinance} onClick={() => openTransaction("Expense")}>＋ Record Expense</button><button className="outline-button" disabled={!canWriteFinance} onClick={() => openTransaction("Transfer")}>⇄ Transfer</button></div>;
  ...
  return <main className="app-shell">
    ...
    <aside id="main-sidebar" className={`sidebar mobile-drawer ${mobileNavOpen?"open":""}`}><div className="brand"><span className="brand-mark">E</span><span>Emmanuel<br /><b>Cash Flow</b></span></div><nav aria-label="Main navigation">{navItems.map(([key, icon, label]) => <button key={key} className={`nav-item ${view === key ? "active" : ""}`} onClick={() => { setView(key); setMobileNavOpen(false); }}><Icon>{icon}</Icon>{label}</button>)}</nav><div className="sidebar-foot"><div className="connection-state"><div className={`sync-dot ${connected ? "" : "pending"}`} /><span>{connected ? "Database connected" : "Connection pending"}</span></div><div className="signed-in-user"><b>{profile?.fullName || profile?.email}</b><small>{profile?.role}</small></div><button className="sign-out-button" onClick={() => void signOut()}>Sign out</button></div></aside>
    <section className="workspace">
      ...
      {view === "transactions" && <div className="transaction-ledgers"><section className="panel table-panel"><div className="panel-head"><div><p className="eyebrow">{data.transactions.length} records</p><h2>Income and expenses</h2></div></div><TransactionTable transactions={data.transactions} emptyMessage="No income or expenses yet." canEdit={canWriteFinance} onDetails={openTransactionDetails} onEdit={openTransactionEdit} /></section><TransferHistory transfers={data.transfers}/></div>}
      {view === "income" && <section className="panel table-panel"><div className="panel-head"><div><p className="eyebrow">{income.length} income records</p><h2>Money received</h2></div><strong className="panel-total income-text">{peso(totals.moneyIn)}</strong></div><TransactionTable transactions={income} emptyMessage="No income has been recorded." canEdit={canWriteFinance} onDetails={openTransactionDetails} onEdit={openTransactionEdit} /></section>}
      {view === "expenses" && <section className="panel table-panel"><div className="panel-head"><div><p className="eyebrow">{expenses.length} expense records</p><h2>Money spent</h2></div><strong className="panel-total">{peso(totals.moneyOut)}</strong></div><TransactionTable transactions={expenses} emptyMessage="No expenses have been recorded." canEdit={canWriteFinance} onDetails={openTransactionDetails} onEdit={openTransactionEdit} /></section>}
      ...
      {view === "access-requests" && profile?.role === "Admin" && <AccessRequestsView />}
...
function TransactionForm({ data, initialType, transaction, saving, onSubmit }: { data: Data; initialType: TransactionType; transaction?:Transaction; saving: boolean; onSubmit: (payload: CashFlowMutation) => Promise<void> }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const value:Transaction={id:transaction?.id||crypto.randomUUID(),source,date:String(form.get("date")||""),type,account:String(form.get("account")||""),category,description:String(form.get("description")||""),vendor:type==="Expense"?String(form.get("vendor")||""):"",moneyIn:type==="Income"?amount:0,moneyOut:type==="Expense"?amount:0,paymentMethod:String(form.get("paymentMethod")||""),reference:String(form.get("reference")||""),notes:String(form.get("notes")||""),specifiedDetails:String(form.get("specifiedDetails")||""),createdAt:transaction?.createdAt||new Date().toISOString()};
    void onSubmit({action:transaction?"updateTransaction":"addTransaction",transaction:value});
  };
  return <form className="record-form" onSubmit={submit}><div className="segmented transaction-types"><button type="button" disabled={!!transaction} className={type==="Income"?"selected":""} onClick={()=>changeType("Income")}>Money in</button><button type="button" disabled={!!transaction} className={type==="Expense"?"selected expense":""} onClick={()=>changeType("Expense")}>Money out</button><button type="button" disabled={!!transaction} className={type==="Transfer"?"selected transfer":""} onClick={()=>changeType("Transfer")}>Transfer</button></div>{type==="Transfer"?<div className="form-grid"><label>From account<select name="fromAccountId" required>{accounts.map(account=><option key={account.id} value={account.id}>{account.name} · {peso(account.currentBalance)}</option>)}</select></label><label>To account<select name="toAccountId" defaultValue={accounts[1]?.id||""} required>{accounts.map(account=><option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label>Date<input name="date" type="date" defaultValue={today} required /></label><label>Amount (PHP)<input name="amount" type="number" min="0.01" step="0.01" placeholder="0.00" required /></label><label>Reference<input name="reference" placeholder="Transfer reference no." /></label><label className="full">Notes<textarea name="notes" rows={3} placeholder="Optional transfer notes" /></label><p className="form-help full">Transfers move money between accounts only. They are excluded from income, expense, net cash flow, analytics, and reports.</p></div>:<div className="form-grid"><label>Date<input name="date" type="date" defaultValue={transaction?.date||today} required /></label><label>Amount (PHP)<input name="amount" type="number" min="0.01" step="0.01" defaultValue={transaction?transaction.moneyIn||transaction.moneyOut:undefined} placeholder="0.00" required /></label><label>Account<select name="account" defaultValue={transaction?.account} required>{accounts.map(account=><option key={account.id} value={account.name}>{account.name}</option>)}</select></label><label>Category<select name="category" value={categoryName} onChange={event=>setCategoryName(event.target.value)} required>{categories.map(category=><option key={category.id} value={category.name}>{category.name}</option>)}</select></label>{isOtherCategory(categoryName)&&<label className="full">Specify details<input name="specifiedDetails" defaultValue={transaction?.specifiedDetails||extractSpecifiedDetails(transaction?.notes||"")} placeholder="Describe the other income or expense" required /></label>}{type==="Expense"&&<label className="full">Vendor / Payee<input name="vendor" defaultValue={transaction?.vendor||""} placeholder="Who was paid?" /></label>}<label className="full">Description<input name="description" defaultValue={transaction?.description||""} placeholder="What was this for?" required /></label><label>Payment method<select name="paymentMethod" defaultValue={transaction?.paymentMethod||"Cash"}><option>Cash</option><option>Bank Transfer</option><option>Check</option><option>Card</option><option>Online</option><option>Other</option></select></label><label>Reference<input name="reference" defaultValue={transaction?.reference||""} placeholder="Receipt or reference no." /></label><label className="full">Notes<textarea name="notes" rows={3} defaultValue={stripSpecifiedDetails(transaction?.notes||"")} placeholder="Optional notes" /></label></div>}{transaction&&<p className="audit-note">This saves over the original record ID. Supabase audit logs retain the previous and updated values.</p>}<button className="primary-button form-submit" disabled={saving||accounts.length<(type==="Transfer"?2:1)||(type!=="Transfer"&&!categories.length)}>{saving?"Saving…":transaction?"Save transaction changes":type==="Income"?"Save income":type==="Expense"?"Save expense":"Save transfer"}</button></form>;
}
