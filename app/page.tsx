...
import { addPayable, addTransaction, createAccount, extractSpecifiedDetails, isOtherCategory, loadCashFlow, recordPayablePayment, stripSpecifiedDetails, updateAccount, updateTransaction } from "../src/services/cashflow";
...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {view === "transactions" && <section className="panel table-panel"><div className="panel-head"><div><p className="eyebrow">{data.transactions.length} records</p><h2>All transactions</h2></div></div><TransactionTable transactions={data.transactions} emptyMessage="No transactions yet." canEdit={canWriteFinance} onDetails={openTransactionDetails} onEdit={openTransactionEdit} /></section>}
...
function TransactionForm({ data, initialType, transaction, saving, onSubmit }: { data: Data; initialType: TransactionType; transaction?:Transaction; saving: boolean; onSubmit: (payload: CashFlowMutation) => Promise<void> }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const form = new FormData(event.currentTarget), amount = Number(form.get("amount")), category=String(form.get("category")||"");
