...
export async function loadCashFlow():Promise<CashFlowData>{
  const db=client();
  const [accountsResult,categoriesResult,offeringsResult,donationsResult,expensesResult,payablesResult,paymentsResult,transfersResult]=await Promise.all([
    ...
    db.from("account_transfers").select("id,transfer_date,from_account_id,to_account_id,amount,reference,notes,recorded_by,created_at,from_account:accounts!account_transfers_from_account_id_fkey(name),to_account:accounts!account_transfers_to_account_id_fkey(name),recorder:users!account_transfers_recorded_by_fkey(full_name,email)").order("transfer_date",{ascending:false}).order("created_at",{ascending:false}),
  ]);
...
export async function addTransfer(transfer:AccountTransferInput,data:CashFlowData){
  ...
  const recordedBy=await currentUserId();
  const {error}=await client().from("account_transfers").insert({id:transfer.id,transfer_date:transfer.date,from_account_id:transfer.fromAccountId,to_account_id:transfer.toAccountId,amount:transfer.amount,reference:transfer.reference,notes:transfer.notes,recorded_by:recordedBy}).select("id").single();
  if(error)throw new Error(error.message);
}
