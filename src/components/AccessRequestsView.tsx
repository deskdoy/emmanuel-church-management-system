...
export function AccessRequestsView() {
  ...
  return <section className="panel table-panel access-request-panel">
    ...
    {loading?<div className="empty-state" role="status"><div className="empty-icon">◷</div><h3>Loading requests</h3><p>Retrieving the secured approval queue…</p></div>:<div className="table-wrap access-table"><table><thead><tr><th>Submitted</th><th>Applicant</th><th>Requested role</th><th>Reason</th><th>Status / Final role</th><th>Action</th></tr></thead><tbody>{visible.map(request=><tr key={request.id}><td>{timestamp(request.createdAt)}</td><td><b>{request.fullName}</b><small>{request.email}{request.phone?` · ${request.phone}`:""}</small></td><td><span className="suggested-role">{request.requestedRole}</span><small>Suggestion only</small></td><td className="request-reason">{request.reason}</td><td><span className={`status request-${request.status.toLowerCase()}`}>{request.status}</span>{request.approvedRole&&<small>Final role: {request.approvedRole}</small>}{request.approvedAt&&<small>{request.approvedByName||"Admin"} · {timestamp(request.approvedAt)}</small>}</td><td>{request.status==="Pending"?<div className="row-actions"><button className="table-action" disabled={!!processingId} onClick={()=>{setApproval(request);setFinalRole("");}}>Approve</button><button className="table-action danger" disabled={!!processingId} onClick={()=>void reject(request)}>{processingId===request.id?"Working…":"Reject"}</button></div>:<span className="processed-label">Processed</span>}</td></tr>)}{!visible.length&&<tr><td colSpan={6} className="blank-row">No {status.toLowerCase()} access requests.</td></tr>}</tbody></table></div>}
