...
export function BackupCenterView({ profile }: { profile: AppUser }) {
  ...
  const refreshHistory = useCallback(async () => {
    ...
    try { setHistory(await loadExportHistory()); }
  ...
  const generate = async () => {
    ...
    try {
      const nextDataset = await loadBackupDataset(type, scope);
      const nextMetadata: BackupExportMetadata = { exportId: createExportId(), organization: "Emmanuel Church", generatedAt: new Date().toISOString(), generatedBy: profile.fullName || profile.email, scopeLabel: scopeLabel(scope) };
      await recordExportActivity(nextDataset, nextMetadata, scope);
  ...
  return <div className="operations-layout backup-center">
    ...
    {dataset && metadata ? <section className="panel backup-preview">
      ...
      <article className="backup-print-sheet">
        <header><div><p>Emmanuel Church</p><h2>{dataset.title} Backup Summary</h2></div><span>Export ID<br/><b>{metadata.exportId}</b></span></header>
